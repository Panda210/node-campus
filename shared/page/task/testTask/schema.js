      ...
      optionsButtons.map((item, index) => {
        ...
        if (record.owner === record.loginUserInfo || record.submitUser === record.loginUserInfo) {
          ...
          if (record.status !== '4' && record.status !== '5' && item.text === '转交') {
            // 任务在resolve或者complete或者close的情况是不能进行操作：转交
            authButtons.push(index);
          }
        }
        // 任务的责任人员可以解决
...
const editSchema = {
  ...
  fields: [{
    ...
    type: FIELD_TYPE.SELECT,
    required: true,
    url: '/api/smart/task/parentList',
    // 查询未完成的测试任务
    param: {
      pageNum: 1, pageSize: 10, param: { type: '1' }, extraInfo: { stateList: ['2', '3'] }
    },
    formatter: (responseData) => {
      if (Array.isArray(responseData.value)) {
        return responseData.value.map(subItem => ({ key: subItem.id, value: subItem.name }));
      }
    }
  }, {
    key: 'grade',
